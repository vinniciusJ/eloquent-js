...
function availableNeighbors(nest) {
    let requests = nest.neighbors.map(neighbor => request(nest, neighbor, "ping").then(() => true, () => false))
...
function findInRemoteStorage(nest, name) {
    ...
    function next() {
        if (sources.length == 0) {
            ...
            let source = sources[ Math.floor(Math.random() * sources.length) ].filter(n => n != source)
...
async function chicks(nest, year) {
    ...
    await Promise.all(network(nest).map(async name => list += `${name}: ${await anyStorage(nest, name, `chicks in ${year}`)}\n`))
